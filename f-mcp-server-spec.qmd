# fw-governance MCP Server Specification {#sec-mcp-server-spec}
...
| **Note** | This appendix is the technical specification for the `fw-governance` MCP server. It is a companion to the governance documents; the normative requirements the server enforces are stated in the domain chapters. For an overview of how the server fits into the adoption process, see @sec-mcp-overview. |
:::
