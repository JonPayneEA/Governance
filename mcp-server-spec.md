...
{
  "topic": "pandas usage",
  "rule": "pandas must not be used in Tier 1 operational tools or in shared modules. It may be used freely in Tier 3 exploratory work and Jupyter notebooks.",
  ...
  "exceptions": "Tier 3 exploratory work and Jupyter notebooks"
}
```
...
```json
{
  ...
  "checks": {
    ...
    "test_coverage": {"status": "FAIL", "detail": "62% line coverage (minimum 70% for Tier 1)", "value": 62},
...
Run the annual Tier 1 audit checks across all registered Tier 1 assets.

| Field | Detail |
|---|---|
| **Description** | Iterates over all Tier 1 entries in the Tool Register and Model Register. For each tool, runs `check_tier_compliance`. For each model, checks condition rating currency and calibration record completeness. Returns a consolidated audit report. |
